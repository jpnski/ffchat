...
Everything needed to turn the `release/` tree into a signed Windows installer
that non-developers can double-click.
...
```powershell
.\release\installer\bootstrap.ps1
...
```powershell
# From anywhere — build.ps1 self-anchors to release/
