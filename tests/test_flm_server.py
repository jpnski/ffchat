...
def _fake_run(stdout="", returncode=0, stderr="", capture=None):
    """Return a run_flm stand-in yielding a fixed CompletedProcess-like object."""
...
def test_installed_returns_only_installed_clean_names(monkeypatch):
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(stdout=SAMPLE))
...
def test_uses_json_mode_not_quiet_text(monkeypatch):
    cap = {}
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(stdout=SAMPLE, capture=cap))
...
def test_tolerates_non_json_preamble(monkeypatch):
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(stdout="loading models...\n" + SAMPLE))
...
def test_decorated_text_without_json_yields_error_not_bogus_models(monkeypatch):
    ...
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(stdout=bad))
    out = flm_server.flm_list("installed", "x")
    assert out["models"] == []
...
def test_nonzero_exit_returns_error(monkeypatch):
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(returncode=1, stderr="boom"))
    out = flm_server.flm_list("installed", "x")
    assert out["models"] == []
...
def test_missing_cli_returns_error(monkeypatch):
    ...
    monkeypatch.setattr(flm_server, "run_flm", _raise)
    out = flm_server.flm_list("installed", "x")
    assert out["models"] == []
...
def test_bad_filter_rejected_before_subprocess(monkeypatch):
    ...
    monkeypatch.setattr(flm_server, "run_flm", _run)
...
def test_falls_back_to_name_prefix_when_metadata_missing(monkeypatch):
    ...
    monkeypatch.setattr(flm_server, "run_flm", _fake_run(stdout=sparse))
    out = flm_server.flm_list("installed", "x")
